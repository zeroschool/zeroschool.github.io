function populateHTML(nPosts){
    for (let i = 0; i<nPosts; i++){
        ...
        `<div class="nes-container with-title is-dark twetch" style="height:auto;"position: relative; border-color: #777; background-color: #000000; margin-bottom: 5px;">
        <div class="twetch-container" >
        ...
        <p class="postContent urlFormat"></p></div>
            <div class="item">
...
async function postsQuery(){
    ...
    for (let i=0; i<posts.length;i++){
        ...
        if ((content.indexOf("twetch.app/t") >= 0) && (content.match(twetchRegex)[0] === content)){
...
function applyURLs(content) {
    if (content.indexOf("https://media.bitcoinfiles.org/") >= 0) {
        ...
        for (word of dict) {
                ...
                let tWord = content.substring(pos,content.length -1);
                if (pos){ 
