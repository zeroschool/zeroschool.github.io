...
function loadingDlg() {
    ...
    dialog.innerHTML =
        `<form method="dialog" style="width: 230px; height:30px"><p id="loading">Generating magic number...</p><menu class="dialog-menu"></menu></form>`;
    let itoken = localStorage.getItem('imbToken');
    if (itoken === null) {
        return
    }
...
async function postsQuery() {
    ...
    let response = await sdk.query(`{
                allPosts(filter: {bContent: {includes: "$21e8"}}, ${selOrder === '2' ? "" : "first: 50,"} ${orderBy}) {
...
posts = response.allPosts.nodes;
    ...
    for (let i = 0; i < posts.length; i++) {
        let content = posts[i].bContent.replace('$21e8', '');
